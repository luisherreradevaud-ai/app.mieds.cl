...
<hr />
<?php 
Msg::show(1,'Producto guardado con &eacute;xito','info');
?>
<form id="productos-form">
  ...
  <div class="row">
    ...
    <div class="col-12 mb-1 d-flex justify-content-between">
      <button class="btn btn-sm btn-danger eliminar-obj-btn"><i class="fas fa-fw fa-trash"></i> Eliminar Producto</button>
  ...
  <div class="modal fade" tabindex="-1" role="dialog" id="eliminar-obj-modal">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <center><h5>Desea eliminar este Producto?<br/>Este paso no es reversible.</h5></center>
        </div>
        <div class="modal-footer">
...
var items = <?= json_encode($obj->productos_items,JSON_PRETTY_PRINT); ?>;
...
$(function() {
  ...
  armarRecetasSelect();
  armarCantidadSelect();
  $('select[name="cantidad"]').val(obj['cantidad']);

  // Cargar configuracion de envases si es Caja
  if(obj.tipo == 'Caja') {
    $('select[name="tipo_envase"]').val(obj.tipo_envase);
    armarFormatosSelect();
    $('select[name="id_formatos_de_envases"]').val(obj.id_formatos_de_envases);
    $('input[name="cantidad_de_envases"]').val(obj.cantidad_de_envases);
    // Cargar es_mixto
    if(obj.es_mixto == 1) {
      $('#es_mixto_check').prop('checked', true);
      toggleMixto(true);
    }
  }
